		...
		if(isset($_POST['itemList'])){
			...
			foreach($itemList as $item){
				...
				if ($itemStmt){
					...
					if($itemRow['Offer'] === '0'){
						$totalReserved = 0;
						foreach(explode(',',$itemRow['Reserved']) as $reserved){
							$totalReserved = $totalReserved + intval(explode('|',$reserved)[0]);
						}
						if($item[1] > ($itemRow['Remaining'] - $totalReserved)){
							$notEnoughItemsLeft = true;
						}
					}else{
						if(explode(']',explode('[',$itemRow['Offer'])[1])[0] === 'bundle'){
							
							$bundleContent = explode('}',explode('{',$itemRow['Offer'])[1])[0];
							foreach(explode(',',$bundleContent) as $bundleItem){
								...
								if ($bundleStmt){
									...
									$totalReserved = 0;
									foreach(explode(',',$bundleRow['Reserved']) as $reserved){
										$totalReserved = $totalReserved + intval(explode('|',$reserved)[0]);
									}
									if($item[1] > ($bundleRow['Remaining'] - $totalReserved)){
										$notEnoughItemsLeft = true;
									}
								}
							}
			...
			foreach($itemList as $item){
				...
				$itemQuery2 = "SELECT * FROM products WHERE Name=?";
				$itemStmt2 = $mysqli->stmt_init();
				$itemStmt2 = $mysqli->prepare($itemQuery2);
								
				if ($itemStmt2){
					$itemStmt2->bind_param('s', $item[0]);
					$itemStmt2->execute();
					$itemResult2 = get_result($itemStmt2);
					$itemRow2 = array_shift($itemResult2);
					
					if($itemRow2['Offer'] !== '0'){
						if(explode(']',explode('[',$itemRow2['Offer'])[1])[0] === 'bundle'){
							foreach(explode(',', explode('}',explode('{',$itemRow2['Offer'])[1])[0]) as $bundleItems){
								...
								if ($bundleStmt){
									$bundleStmt->bind_param('s', $bundleItems);
									$bundleStmt->execute();
									$bundleResult = get_result($bundleStmt);
									$bundleRow = array_shift($bundleResult);
									
									$reservedText = $bundleRow['Reserved'];
									$date = new DateTime();
									...
									if ($reservationStmt){
										$reservationStmt->bind_param('ss', $reservedText, $bundleItems);
							...
							$reservedText = $itemRow2['Reserved'];
							$date = new DateTime();
							...
							$reservationQuery2 = "UPDATE products SET Reserved=? WHERE Name=?";
							$reservationStmt2 = $mysqli->stmt_init();
							$reservationStmt2 = $mysqli->prepare($reservationQuery2);
									
							if ($reservationStmt2){
								$reservationStmt2->bind_param('ss', $reservedText, $item[0]);
								if($reservationStmt2->execute())
								{
									$_SESSION['reservation'] = $date->getTimestamp();
								}
							}
						...
						$reservationQuery3 = "UPDATE products SET Reserved=? WHERE Name=?";
						$reservationStmt3 = $mysqli->stmt_init();
						$reservationStmt3 = $mysqli->prepare($reservationQuery3);
									
						if ($reservationStmt3){
							$reservationStmt3->bind_param('ss', $reservedText, $item[0]);
							if($reservationStmt3->execute())
							{
								$_SESSION['reservation'] = $date->getTimestamp();
							}
						}
	...
	<form class="checkoutForm" action="checkoutStep2.php" method="POST">
		...
		if(isset($_POST['promo']) || isset($_GET['promo'])){
			if(isset($_POST['promo'])){
				echo "<tr><td><input type='hidden' name='promo' value='".$_POST['promo']."' /></td></tr>";
			}else{
				echo "<tr><td><input type='hidden' name='promo' value='".$_GET['promo']."' /></td></tr>";
			}
		}
		?>
		</table>
